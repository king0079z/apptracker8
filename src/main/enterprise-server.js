...
const Datastore = require('nedb'); // Changed from sqlite3
...
class EnterpriseServer {
  constructor(pullClient, dataManager) {
    ...
    this.dbPath = path.join(electronApp.getPath('userData'), 'nedb-data');
    
    // Initialize NeDB datastores instead of SQLite
    this.db = {
      historicalData: null,
      costTracking: null,
      alerts: null
  ...
  async initDatabase() {
    ...
    this.db.historicalData = new Datastore({
      filename: path.join(this.dbPath, 'historical_data.db'),
      autoload: true,
      timestampData: true
    });

    this.db.costTracking = new Datastore({
      filename: path.join(this.dbPath, 'cost_tracking.db'),
      autoload: true
    });

    this.db.alerts = new Datastore({
      filename: path.join(this.dbPath, 'alerts.db'),
      autoload: true,
      timestampData: true
    });

    // Create indexes for better performance
    this.db.historicalData.ensureIndex({ fieldName: 'client_id' });
    this.db.historicalData.ensureIndex({ fieldName: 'timestamp' });
    this.db.costTracking.ensureIndex({ fieldName: 'month' });
    this.db.costTracking.ensureIndex({ fieldName: 'department' });
    this.db.alerts.ensureIndex({ fieldName: 'client_id' });
    this.db.alerts.ensureIndex({ fieldName: 'resolved' });

    // Compact databases on startup
  ...
  stop() {
    ...
    // NeDB doesn't need explicit closing
  }
  ...
  compactDatabases() {
    Object.values(this.db).forEach(datastore => {
      if (datastore && datastore.persistence) {
  ...
  async storeHistoricalData(clientId, department, data) {
    return new Promise((resolve, reject) => {
      this.db.historicalData.insert({
        client_id: clientId,
        department: department,
        data: data, // NeDB can store objects directly
        timestamp: new Date()
      }, (err, newDoc) => {
        if (err) reject(err);
        else resolve(newDoc);
  ...
  async updateCostTracking(costAnalysis) {
    ...
    for (const dept of costAnalysis.byDepartment) {
      await new Promise((resolve, reject) => {
        // Update or insert using NeDB
        this.db.costTracking.update(
          { month: month, department: dept.name },
          {
            month: month,
            department: dept.name,
            total_cost: dept.totalCost,
            active_licenses: dept.activeLicenses,
            unused_licenses: dept.unusedLicenses
          },
          { upsert: true },
