    ...
    Object.values(apps).forEach(app => {
        if (app.sessions) {
            app.sessions.forEach(session => {
                ...
                if (daysAgo <= 7) {
                    thisWeek += session.duration || 0;
                } else if (daysAgo <= 14) {
                    lastWeek += session.duration || 0;
                }
            });
        }
    });
...
function findMostUsedApp(apps) {
    ...
    Object.entries(apps).forEach(([name, data]) => {
        if (data.totalUsage > maxUsage) {
            maxUsage = data.totalUsage;
            mostUsed = {
                name: name,
                hours: Math.round(data.totalUsage / 60)
            };
        }
    });
...
function analyzeHourlyPattern(apps) {
    ...
    const peakHour = `${maxHour}:00 - ${maxHour + 1}:00`;
...
function calculateUtilizationMetrics(apps, plugins) {
    ...
    const trend = state.historicalData.length > 1 ? 
        ...
        <div class="comparison-bars">
            <div class="comparison-item">
                ...
                <div class="comparison-bar-container">
                    <div class="comparison-bar this-week" style="width: ${(weeklyData.thisWeek / max) * 100}%">
            ...
            <div class="comparison-item">
                ...
                <div class="comparison-bar-container">
                    <div class="comparison-bar last-week" style="width: ${(weeklyData.lastWeek / max) * 100}%">
