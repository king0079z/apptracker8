...
// Handle missing enterprise-client module gracefully
let EnterpriseClient;
try {
  EnterpriseClient = require('./enterprise-client');
} catch (error) {
  console.log('Enterprise client module not found, running in standalone mode');
  // Create a stub class
  EnterpriseClient = class {
    ...
    async syncData() {
      return { success: false, message: 'Enterprise module not installed' };
    }
  };
}
...
class SoftwareMonitor {
  ...
  async init() {
    ...
    // Initialize enterprise client if configured and module exists
    if (this.dataManager.usageData?.metadata?.isEnterprise && EnterpriseClient) {
  ...
  setupIpcHandlers() {
    ...
    ipcMain.handle('get-usage-data', async () => {
      ...
      console.log('Sending usage data to renderer');
      return data;
    });
